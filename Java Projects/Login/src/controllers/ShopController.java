...
public class ShopController {
	...
	public void openShopMealFrame(JFrame frame) {
		...
		try {
			...
			JOptionPane.showMessageDialog(null, "Errore critico, contattare l' amministratore", "Error",
	...
	public void openShopViewOrdersFrame(JFrame frame) {
		...
		try {
			...
			if (!all_orders.isEmpty()) {
				...
				JOptionPane.showMessageDialog(null, "Lo shop selezionato non ha ordini completati", "Warning",
	...
	public void openAdminRiderFrame() {
		...
		if (!employed_rider_list.isEmpty()) {
			...
			JOptionPane.showMessageDialog(null, "Lo shop selezionato non ha rider in servizio", "Warning",
					JOptionPane.WARNING_MESSAGE);
		return;
	}
	...
	public void openShopDeliveringOrdersFrame(JFrame frame) {
		...
		try {
			...
			if (!in_delivery_order_list.isEmpty()) {
				...
				frame.setVisible(true);
				JOptionPane.showMessageDialog(null, "Lo shop selezionato non ha ordini in consegna", "Warning",
						JOptionPane.WARNING_MESSAGE);
			}
		} catch (DAOException e) {
			JOptionPane.showMessageDialog(null, "Errore critico, contattare l' amministratore", "Errore",
	...
	public void openShopPendingOrdersFrame(JFrame frame) {
		...
		try {
			...
			if (!pending_order_list.isEmpty()) {
				table.initializePendingOrderTable(shop_pending_orders_frame.getPending_orders_model(),
						pending_order_list);
				...
				JOptionPane.showMessageDialog(null,
						"Lo shop selezionato non ha ordini in attesa di essere ritirati dai rider", "Warning",
						JOptionPane.WARNING_MESSAGE);
			}
		} catch (DAOException e) {
	...
	public void addMealToMenu(ShopMealFrame shop_meal_frame) {
		...
		if (!shop_meal_frame.getAll_meals_table().getSelectionModel().isSelectionEmpty()) {
			try {
				Meal meal = meal_dao
						.getMealByName(shop_meal_frame.getAll_meals_model().getValueAt(row_selected, 0).toString());
				...
				JOptionPane.showMessageDialog(null,
						"Non e' stato possibile aggiungere il pasto al menu', contattare l' amministratore", "Errore",
						JOptionPane.ERROR_MESSAGE);
			}
		} else
	...
	public void addRider(ShopRiderFrame shop_rider_frame) {
		...
		try {
			...
			if (!CF.equals("Errore!")) {
				Rider rider = new Rider(CF, name, surname, birth_date, birth_place_town, gender, cellphone,
						new Address(address, address_civic_number, cap, address_town, province_town), vehicle,
						working_hours, (short) 0);
				rider_dao.insertRider(rider, current_shop_email);
				shop_rider_frame.getModel().insertRow(shop_rider_frame.getModel().getRowCount(),
						new Object[] { CF, name, surname, input_util.formatDate(birth_date), birth_place_town,
								rider.getAddress().toString(), gender, cellphone, vehicle, working_hours, (short) 0 });
				JOptionPane.showMessageDialog(null, "Rider assunto con successo");
			}
		} catch (DAOException e) {
			JOptionPane.showMessageDialog(null,
					"Riempi correttamente i campi.\nHint: Controlla la validita dell' indirizzo, data di nascita, orario lavorativo e cellulare",
	...
	public void removeMealFromMenu(ShopMealFrame shop_meal_frame) {
		...
		if (row != -1) {
			...
			try {
				...
				shop_meal_frame.getAll_meals_model()
						.addRow(new Object[] { meal_to_remove.getName(), meal_to_remove.getCategory(),
								meal_to_remove.getPrice(), meal_to_remove.getIngredients(),
								input_util.arrayListToTokenizedString(meal_to_remove.getAllergen_list(), ", ") });
			...
			JOptionPane.showMessageDialog(null, "Seleziona il pasto da rimuovere", "Warning",
					JOptionPane.WARNING_MESSAGE);
		return;
	}
	...
	public void updateRider(ShopRiderFrame shop_rider_frame) {
		...
		if (row != -1) {
			...
			JOptionPane.showMessageDialog(null, "Seleziona il rider da aggiornare", "Errore",
					JOptionPane.ERROR_MESSAGE);
		return;
	}
	...
	public void linkRiderToOrder(ShopPendingOrdersFrame shop_pending_orders_frame) {
		...
		if (shop_pending_orders_frame.getPending_orders_table().getSelectionModel().isSelectionEmpty()
				|| shop_pending_orders_frame.getRiders_table().getSelectionModel().isSelectionEmpty())
			JOptionPane.showMessageDialog(null, "Seleziona un ordine da associare ad un rider", "Errore",
					JOptionPane.ERROR_MESSAGE);
		else {
			...
			try {
				Order order = order_dao.getOrderById(shop_pending_orders_frame.getPending_orders_model()
						.getValueAt(selected_order_row, 0).toString());
				Rider rider = rider_dao.getRiderByCf(
						shop_pending_orders_frame.getRiders_model().getValueAt(selected_rider_row, 0).toString());
				order_dao.linkRiderToOrder(order, rider);
				shop_pending_orders_frame.getPending_orders_model().removeRow(selected_order_row);
				shop_pending_orders_frame.getRiders_model().setValueAt(Integer.parseInt(
						shop_pending_orders_frame.getRiders_model().getValueAt(selected_rider_row, 4).toString()) + 1,
						selected_rider_row, 4);
				if (shop_pending_orders_frame.getRiders_model().getValueAt(selected_rider_row, 4).toString()
						.equals("3"))
					shop_pending_orders_frame.getRiders_model().removeRow(selected_rider_row);
			} catch (DAOException e) {
				JOptionPane.showMessageDialog(null, "Non e' stato possibile associare l' ordine al rider", "Errore",
						JOptionPane.ERROR_MESSAGE);
			}
		}
		return;
		...
		if (row != -1) {
			try {
				...
				shop_delivering_orders_frame.getModel()
						.removeRow(shop_delivering_orders_frame.getTable().getSelectedRow());
	...
	// Metodo che chiude solo gli statement usati dallo shop, quando ci si
	// disconnette dallo shop frame e si ritorna al login frame

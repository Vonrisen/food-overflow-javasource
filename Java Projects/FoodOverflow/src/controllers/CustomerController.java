...
public class CustomerController {
		...
		if (row != -1) {
			...
			try {
				...
				if (new_quantity > 0) {
					...
					JOptionPane.showMessageDialog(null, "Inserire una quantita' positiva", "Errore",
							JOptionPane.ERROR_MESSAGE);
			} catch (NumberFormatException n) {
				JOptionPane.showMessageDialog(null, "Inserire un prezzo valido", "Errore", JOptionPane.ERROR_MESSAGE);
			}
	...
	public void openCustomerCheckoutFrame(CustomerCartFrame customer_cart_frame) {
		...
		customer_cart_frame.setVisible(false);
		return;
	}
	...
	public void updateCustomerCF(CustomerProfileFrame customer_profile_frame) {

		String old_cf = customer.getCf();
		customer.setCf(customer_profile_frame.getFiscal_codeTF().getText());
		try {
			customer_dao.updateCustomerCF(customer, old_cf);
			JOptionPane.showMessageDialog(null, "Codice fiscale aggiornato correttamente");
		} catch (DAOException e) {
	...
	public void updateDeliveryAddress(CustomerProfileFrame customer_profile_frame) {
	        ...
	        if(!customer_profile_frame.getAddress_provinceCB().getSelectedItem().equals("Seleziona provincia")&&!customer_profile_frame.getAddress_provinceCB().getSelectedItem().equals("-------------------")
	                &&!customer_profile_frame.getAddress_townCB().getSelectedItem().equals("Seleziona comune")) {
	            ...
	            try {
	                ...
	                JOptionPane.showMessageDialog(null,
	                "Riempire i campi correttamente.", "Errore",
	                JOptionPane.ERROR_MESSAGE);
	            }

	        } else    
	        	JOptionPane.showMessageDialog(null,
	                "Riempire i campi correttamente.", "Errore",JOptionPane.ERROR_MESSAGE);
	}

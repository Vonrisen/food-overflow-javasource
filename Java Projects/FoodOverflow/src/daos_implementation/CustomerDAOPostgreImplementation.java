...
public class CustomerDAOPostgreImplementation implements CustomerDAO {
	...
	private PreparedStatement get_all_customers_PS, insert_customer_PS, authenticateCustomerLogin_PS,
			get_customer_by_email_PS, update_customer_address_PS, update_customer_password_PS, update_customer_CF_PS;
	...
	public void updateCustomerCF(Customer customer, String old_cf) throws DAOException{
		...
		try {
			update_customer_CF_PS.setString(1, customer.getCf());
			update_customer_CF_PS.setString(2, old_cf);
			update_customer_CF_PS.executeUpdate();
		}catch (SQLException e) {
			throw new DAOException();
		}
		return;
